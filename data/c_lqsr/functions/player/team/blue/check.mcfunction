execute if data storage c_lqsr:status {Players:{Blue:3}} run tellraw @s {"text": "該当のチームは満員です","color": "red"}
...
execute if predicate c_lqsr:is_not_joined_team unless data storage c_lqsr:status {Players:{Blue:3}} run function c_lqsr:player/team/blue/success